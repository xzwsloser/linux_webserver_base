...
int main(int argc, char **argv) {
  ...
  struct event *signal_ev =
      event_new(base, SIGINT, EV_SIGNAL | EV_PERSIST, signal_cb, NULL);
  struct event *listen_ev =
      event_new(base, lfd, EV_READ | EV_PERSIST, io_cb, NULL);
  // 3. 添加到事件处理队列中
...
void io_cb(int fd, short event, void *argc) {
  ...
  printf("Connect to:[client]%s:%d\n",
         inet_ntop(AF_INET, (void *)&client_addr.sin_addr.s_addr, buf,
                   client_addr_len),
  ...
  users[index].client_ev =
      event_new(base, cfd, EV_READ | EV_PERSIST, client_cb, (void *)index);
